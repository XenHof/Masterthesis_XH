...
#Questionnaire 1/4
# load the data from the first questionnaire
data1 = read_csv("../data/nodwlib1_4.csv") 
nrow(data1) #35 participants
head(data1)
summary(data1) 


# Step 2: Replace Timestamp with Participant ID
colnames(data1)[1] <- "participantID" # Rename the column
data1$participantID <- match(data1$participantID, unique(data1$participantID)) # Assign unique IDs
print(table(data1$participantID)) # View distribution of participant IDs

# Step 3: Rename Columns
colnames(data1)[2] <- "prisonVSeducationprogram"
colnames(data1)[3] <- "feel-strongly"
colnames(data1)[4] <- "trans-stereotypes-confused"
colnames(data1)[5] <- "trans-stereotypes-mentally-ill"
colnames(data1)[6] <- "trans-stereotypes-dangerous"
colnames(data1)[7] <- "trans-stereotypes-frauds"
colnames(data1)[8] <- "trans-stereotypes-unnatural"
colnames(data1)[9] <- "cis-stereotypes-confused"
colnames(data1)[10] <- "cis-stereotypes-mentally-ill"
colnames(data1)[11] <- "cis-stereotypes-dangerous"
colnames(data1)[12] <- "cis-stereotypes-frauds"
colnames(data1)[13] <- "cis-stereotypes-unnatural"
colnames(data1)[14] <- "gender-fairness-apartment"
colnames(data1)[15] <- "gender-fairness-sports"
colnames(data1)[16] <- "gender-fairness-street-harassment"
colnames(data1)[17] <- "gender-fairness-bullying"
colnames(data1)[18] <- "general-fairness-education"
colnames(data1)[19] <- "general-fairness-healthcare"
colnames(data1)[20] <- "fear-number-of-trans-people"
colnames(data1)[21] <- "fear-all-problems"
colnames(data1)[22] <- "equality-equal-chance"
colnames(data1)[23] <- "equality-shouldnt-worry"
colnames(data1)[24] <- "person-age"
colnames(data1)[25] <- "person-sexual-orientation"
colnames(data1)[26] <- "person-gender"
colnames(data1)[27] <- "person-cis-trans"
